...
var dateFormat = function () {
    ...
    return function (dateArg, mask, utc) {
        var dF = dateFormat;
...
var Formatter = (function () {
    return {
        asValueType: function (value, type, symbol) {
            symbol = symbol || type;
        ...
        capitalize: function (str, checkFirstLetter) {
            if (str) {
                str = str + '';
        ...
        diffTimeSpan: function (later, sooner) {
        ...
        displayTime: function (date, minutesToOffset) {
            if (!date.getDate) {
        ...
        isValid24hTime: function (value) {
            ...
            if (parts.length !== 2 ||
                ...
                parts[0].length !== 2 ||
                parts[1].length !== 2 ||
                ( ((parts[0]*1) > 23) || ((parts[0]*1) < 0) ) ||
                ( ((parts[1]*1) > 59) || ((parts[1]*1) < 0) )
            ) {
                return false;
            }
            return true;
      ...
      money = Math.round((money / 1000)) / 100;
        ...
        msAsDaysHoursMinutes: function (ms) {
            var msInMin = 1000 * 60;
            ...
            if (minutes) {
                var str = '';
                if (days) { str += days + 'd '; }
                if (hours) { str += this.padLeftWith(hours, 0, 2) + 'h '; }
        ...
        minAsHoursMinutes: function (min) {
            if (min) {
                var minInHour = 60;
        ...
        padLeftWith: function (origStr, padStr, maxLength) {
            return _padWith(origStr, padStr, maxLength, true);
        },

        padRightWith: function (origStr, padStr, maxLength) {
        ...
        replaceFrom: function (key, values, options) {
            var i = 0, result = '', prop = null, a = 0;
        ...
        timeFormat: function (str, mask) {
            var timeParts = str.split(':');
            var d = new Date(Date.UTC(0, 0, 0, timeParts[0], timeParts[1]));
            return dateFormat(d, mask, true);
        },
        ...
        timeSpan: function (val) {
            ...
            if (!val.toUTCString) { //If not a date should convert to one.
                val = _parseStrToDate(val);
            }

            if (val.toUTCString) { //Make sure that I have a date now.
        ...
        timeSpanFromDate: function (date) {
        ...
        truncate: function (str, limit, backtrack) {
        ...
        itemIdLink: function(itemId, showItemIdLink, type)
        {
            if (showItemIdLink == 'true')
            {
                if (type === 'ticket')
                {
                    return '<a href="/tickets/' + itemId + '">' + itemId + '</a>'
                }
                if (type === 'refund')
                {
                    return '<a href="/tickets/refunds/' + itemId + '">' + itemId + '</a>'
                }
            }
            return itemId;
        },
    ...
    function _utcTime(timeParts) {
        var time = new Date(0, 0, 0, 0, 0, 0, 0);
        time.setUTCHours(timeParts.hours);
        time.setUTCMinutes(timeParts.minutes);
        time.setUTCSeconds(timeParts.seconds);
